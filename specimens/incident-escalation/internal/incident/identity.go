...
// Clock is the injectable passage of time consumed by every policy decision.
...
func RandomSecret() (string, error) {
	value := make([]byte, 32)
	if _, err := rand.Read(value); err != nil {
		return "", fmt.Errorf("generate responder secret: %w", err)
	}
...
func validateSlug(kind, value string) error {
	if !slugPattern.MatchString(value) {
		return fmt.Errorf("%w: %s id must be a lowercase slug of at most 64 characters", ErrInvalid, kind)
	}
	return nil
}
