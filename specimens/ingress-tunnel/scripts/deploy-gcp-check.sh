...
base64encode(templatefile("startup.sh", {project="test-project",domain="tunnel.example.com",acme_email="ops@example.com",admin_actor="proof",bucket="test-artifacts",server_key="server-test",caddy_key="caddy-test",admin_secret="admin",read_secret="read"}))
EXPR
...
shellcheck -e SC1091,SC2154 "$work_dir/cloud.sh" "$work_dir/render-env.sh"
