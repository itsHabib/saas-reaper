...
Status: **the unchanged delegated-domain deployment plus free Cloudflare proxy is
not viable**. This is a research result and an opt-in egress probe, not a new
Terraform pack. No infrastructure, DNS, firewall, runtime defaults, or credentials
change. Source review date: 2026-09-08. Live acceptance below is unperformed.
...
## Smaller candidate worth testing

Use an explicitly selected dedicated registered domain on Cloudflare full setup:
`example.com:8443` for control and `slack.example.com:443` for a claim. Keep the
server's configured domain `example.com`. Apex and wildcard proxied AAAA records
would point to the origin's external IPv6. Do not reuse the delegated child pack
unchanged or move an existing domain's authority as part of this experiment.
...
confirm the chosen E2 size and region with an isolated plan and actual boot.
...
Cloudflare supports HTTPS on 8443 and WebSockets on all plans. This makes control
transport plausible, but does not prove a corporate firewall permits 8443.
...
both apex control and wildcard edge. Origin CA certificates are not a substitute
...
Do not trust arbitrary `CF-Connecting-IP` or `X-Forwarded-For`. A later implementation
must restrict trusted proxies in Caddy, retain the server's loopback-only trust,
and reject spoofing through both direct IPv6 and proxied paths. Until then the
current Caddy may report the Cloudflare peer instead of the visitor.
...
sources, redirects, the selected ACME issuer, DNS propagation checks, metadata
credentials, guest-agent updates and authenticated API operations still require
a cold-boot proof. Private Google Access can cover eligible Google APIs; it is
not general internet IPv4 translation. If a required non-Google endpoint lacks
...
3. On only the new test domain, verify authoritative DNS and proxied A/AAAA,
...
hypothesis. The dedicated-domain candidate remains **unproven**, not deployed.
